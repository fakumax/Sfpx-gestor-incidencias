...
export interface IDatasourceState<TItem extends Etiqueta> {
    items: Array<TItem>;
    item?: TItem;
    isLoading: boolean;
    error: boolean;
...
export type DatasourceHook<TItem extends Etiqueta> = [
...
function useEtiquetaDataSource<TItem extends Etiqueta>(
    ...
    const getItems = handleAsync(async () => {
        dispatch({ type: LOAD });
        const itemsResult = await datasource.getAll();
...
export default useEtiquetaDataSource;
